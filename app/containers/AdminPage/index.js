...
export default function AdminPage() {
  ...
  const onChangeFileHandler = (file, name, setBgGreen, setResetBgTime) => {
    ...
    axios.post('http://157.245.245.34:3000/uploadAdminKeyfile', data, {}).then(res => {
  ...
  const onEjectFileHandler = (name, setResetBgTime, inputRef) => {
    const data = new FormData();
    data.append('filename', name + '.json');
    axios.post('http://157.245.245.34:3000/removeAdminKeyfile', data, {}).then(res => {
  ...
  useInterval(async () => {
    let res = await axios.post('http://157.245.245.34:3000/pollAdminKeyfiles', {}, {});
