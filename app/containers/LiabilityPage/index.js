...
const verifyLiability = async (claim, sig, vestDate) => {
...
export default function AssetPage(props) {
    ...
    const [myReserves, setMyReserves] = useState(null);
    ...
    const loadKey = async () => {
        if (props.location.state!=null && props.location.state.key!=null) {
            ...
            for (let i=0;i<props.location.state.feed.assets.length;i++) {
                verified[props.location.state.feed.assets[i].claimName] = await verifyLiability(props.location.state.feed.assets[i].claim.data, props.location.state.feed.assets[i].claim.fromSignature, props.location.state.feed.assets[i].vestDate);
            }
            setFeed(props.location.state.feed);
            let r = await getReservesAmount(props.location.state.feed.reserves.address);
    ...
    return (
        ...
        <div className="outerDiv center">
            {
            feed.liabilities.map(({ amount, recipient, vestDate, claim, claimName, nonce }) => {
            return <TransactionBlob 
                recipient = {recipient}
                amount = {amount}
                vestDate = {vestDate}
                claimData = {claim.data}
                author = {{id: feed.id, commonName: feed.commonName, reserves: feed.reserves}}
                reservesAmt = {myReserves}
